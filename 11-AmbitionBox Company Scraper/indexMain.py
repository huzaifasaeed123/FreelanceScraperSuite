...
import pandas as pd
# URL of the service
def getsingleRequest(payload,headers,Main_list,page):
    try:
...
for category in categories:
    ...
    for rating in ratings:
        ...
        for index_search in range(1,51):
            ...
            location_list=getRequestCount(payload_Search)["locations"]
            for location in location_list:
                location_Name=location["urlName"]
                Location_Count=location["docCount"]
                print(f"Stats For Location:: where Categories is {category} And Rating is {rating} And Location is {location}")
                print(Location_Count,location_Name)
                if True:
                    main_count=main_count+Location_Count
                    loop_range=int(Location_Count/20)+2
                    if loop_range >500:
                        loop_range=500
                    for page in range(1,loop_range):
                        ...
                        getsingleRequest(payload,headers,Main_list,page)
                else:
                    EmployeeCounts = [
                        "100001", 
                        "50001-100000", 
                    ...
                    for i in range(0,2):
                        payload_emp = {
                            'indianEmployeeCounts': EmployeeCounts,
                            'isFilterApplied': True,
                            'knownFor': [category],
                            'locations':[location_Name],
                            'limit': 20,
                        ...
                        if i==1:
                            payload_emp['indianEmployeeCounts'] = EmployeeCounts
                            data_list=getRequestCount(payload_emp)["indianEmployeeCounts"]
                            for data in data_list:
                                Emp_Count=data["docCount"]
                                main_count=main_count+Emp_Count
                                loop_range2=int(Emp_Count/20)+2
                                Emp_Name=data["urlName"]
                                for index_Emp in range(1,loop_range2):
                                    payload_emp2 = {
                                        'isFilterApplied': True,
                                        'knownFor': [category],
                                        'locations':[location_Name],
                                    ...
                                    getsingleRequest(payload_emp2,headers,Main_list,index_Emp)

                        else:
                            payload_emp['globalEmployeeCounts'] = EmployeeCounts
                            data_list=getRequestCount(payload_emp)["globalEmployeeCounts"]
                            for data in data_list:
                                Emp_Count2=data["docCount"]
                                main_count=main_count+Emp_Count2
                                loop_range2=int(Emp_Count2/20)+2
                                Emp_Name=data["urlName"]
                                for index_Emp in range(1,loop_range2):
                                    payload_emp2 = {
                                        'isFilterApplied': True,
                    ...
                    # pass  # Print or further process the data
        #         break
        #     break
        # break
    break
