# This is the main file which haveall code for This project
# It have two steps,First is to retrive Author INformation 
# And Then retrive all QOUTES Information for particular author
# But we need to apply Threading To All This process,so we are dividing Our Code into Two PArts to apply Threading
# So the file 2-ThreadqoutePart1.py is responsibel to retrive All Author Information with their Qoutes Link
# And The File 3-ThreadQoutePart2.py is responsible to retrive all Qoutes information 
#In Both file,we have apply threading Separately
import requests
import json
...
def retrieveAuthorInformation():
    ...
    if response.status_code == 200:
        ...
        print("Failed")
def retriveAllAuthorQoutes(Author_data):
    Main_data=[]
    index=0
    ...
    for author in Author_data:
        ...
        if response.status_code==200:
            ...
            for qoutelink in QouteList:
                ...
                merged_obj = {**obj1, **author,**obj3,**obj4}
                Main_data.append(merged_obj)
        if index==1000:
            break
    return Main_data
def retriveQoute(url1):
    ...
    if response.status_code==200:
        ...
        QouteText=soup.find('div',class_="quote-text").text
        # print(QouteText)
...
objects_to_excel(listdata,"Qoudata.xlsx")
