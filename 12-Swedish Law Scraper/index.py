...
def getpreface(soup):
    ...
    dates = re.findall(r'\d{1,2}\s+\w+\s+\d{4}', last_paragraph_text)
...
def handle_article(article, mainDatalist, levelList, csvList,date,row):
    # try:
        ...
        pattern = re.compile(r'(Art\.\s*\d+[a-zA-Z]*(?:[–-]\d+[a-zA-Z]*|(?:\sand\s\d+[a-zA-Z]*))*)\s*(.*)')
        # print("Aricle")
        match = pattern.match(heading_text)
        if match:
            ArtNo = match.group(1)
            ArtTag = match.group(2)
...
def extract_date_from_url(url):
    ...
    date_pattern = re.compile(r'/(\d{8})/en/html/fedlex-data-admin-ch-eli-cc-\d+-[^-]+-\d{8}-en-html(-\d+)?\.html$')
...
createCSVFile(csvList, "FinalData3.csv")

df = pd.DataFrame(missed_data)

# Write the DataFrame to a CSV file
df.to_csv('Missing3.csv', index=False)
