...
def getpreface(soup):
    ...
    first_paragraph_text = preface_div.find('p', class_='srnummer').text.strip()
    ...
    dates = re.findall(r'\b\d{1,2}(?:er)?\s+\w+\s+\d{4}\b', last_paragraph_text)
    print(dates)
    ...
    if(len(formatted_dates)==1):
        return [h1_text, first_paragraph_text, formatted_dates[0],"not found"]
    elif(len(formatted_dates)==0):
        return [h1_text, first_paragraph_text, "not found","not found"]
    else:
        return [h1_text, first_paragraph_text,Abbreviation, formatted_dates[0], formatted_dates[1]]
...
def handle_article(article, mainDatalist, levelList, csvList,date,row):
    # try:
        ...
        pattern = re.compile(r'(Art\.|\§)\s*(\d+[a-zA-Z]*(?:\s*à\s*\d+[a-zA-Z]*|\s*et\s*\d+[a-zA-Z]*)?)\s*(.*)')
...
def extract_date_from_url(url):
    #Here is the main part which updated as per language Data
    date_pattern = re.compile(r'/(\d{8})/fr/html/fedlex-data-admin-ch-eli-cc-\d+-[^-]+-\d{8}-fr-html(-\d+)?\.html$')
...
df = pd.read_csv('extracted_data2fr.csv')
start_range=1
end_range=500
...
for index, row in df.iterrows():
    if index>=start_range and index<=end_range:
        ...
        try:
            ...
            # url="https://www.fedlex.admin.ch/filestore/fedlex.data.admin.ch/eli/cc/54/757_781_799/20240701/fr/html/fedlex-data-admin-ch-eli-cc-54-757_781_799-20240701-fr-html-7.html"
...
df.to_csv(f"MissingFr({start_range}-{end_range}).csv", index=False)
