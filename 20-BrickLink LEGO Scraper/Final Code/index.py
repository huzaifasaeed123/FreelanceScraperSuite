...
from parts_parser import parse_all_parts_data
...
CONFIG = {
    ...
    'parts': {
        ...
        'parse_color_table': 'part_parse_color',  # New table for colors
    ...
    try:
        ...
        for category, config in CONFIG.items():
            ...
            if category == 'parts':
                ...
                logger.info(f"Using specialized parts parser for {category}...")
                ...
                logger.info(f"Parsed {len(parsed_data)} {category} items with {len(color_data)} color entries")
            else:
