...
def parse_all_data(raw_table, parsed_table, color_table, batch_size=1000, output_excel=None):
...
def scrape_bricklink_data(output_excel=None):
    """Main function to scrape Bricklink data for parts."""
    logger.info("Starting Bricklink parts data processing")
    
    # Connect to source database (contains raw HTML data)
    db_raw = dataset.connect("sqlite:///bricklink_data_part.db")
    raw_table = db_raw["part"]
    
    # Connect to target database (for parsed data)
    db_parse = dataset.connect("sqlite:///bricklink_parse.db")
    
    # Create or get tables for parsed data
    parts_table = db_parse["part_parse"]
    color_table = db_parse["part_parse_color"]
    
    
    # Process the data
    parts_data, color_data = parse_all_data(
        raw_table, 
        parts_table, 
        color_table, 
    ...
    logger.info(f"Completed processing {len(parts_data)} parts with {len(color_data)} color entries")
    return parts_data, color_data

if __name__ == "__main__":
    # Set the output Excel file path
    output_path = "bricklink_parts_data.xlsx"
    
    # Run the scraper
    scrape_bricklink_data(output_excel=output_path)