...
def retriveurl_branches(obj1,index):
    try:
        base_url="https://www.moneyhouse.ch/de/company/"
        url = f"{obj1['Url']}"
        ...
        if response.status_code == 200:
            ...
            listurls.append(url)
            soup = BeautifulSoup(response.content, "html.parser")
            all_icon=soup.find_all("span",class_="company-status-icon")
            for icon in all_icon:
                sibling_span=icon.find_next("span")
                if sibling_span:
                    new_url=sibling_span.find('a').get('href')
                    listurls.append(f"https://www.moneyhouse.ch{new_url}")

            print(f"Reached at index: {index}")
            # key_classes=soup.find_all("h4",class_="key")
            # # 
            # for key in key_classes:
            #     if "Alter der Firma" in key.text:
...
ScrapedData=readCSV("matched.csv")
Main_list=[]
MissingData=[]
...
df = df.astype(str)
df.to_csv(f"FinalData{program}.csv", index=False)
...
df1 = df1.astype(str)
df1.to_csv(f'MissingFinal{program}.csv', index=False)
# print(list1[0]["Url"])