...
def separate_number_and_text(text):
    # Regular expression to match up to 7-digit numbers at the start
    pattern = r'^(\d{1,7})\s*(.*)$'
    ...
    if match:
        # If a match is found, extract the number and remaining text
        number = match.group(1)
        remaining_text = match.group(2).strip()  # Strip removes leading/trailing whitespace
    else:
        # If no match, return empty number and the full text as remaining
        number = ""
...
def checlslash(text):
    if text=="-":
        ...
        return text
def retriveIndividualPageswithAdditionalData(obj1,index):
    try:
        ...
        if response.status_code == 200:
            ...
            managment_type = ', '.join(sel.xpath('//h4[text()="neueste Verwaltungsräte"]/following-sibling::p/a/text()').getall())
            ...
            for key in key_classes:
                if "Alter der Firma" in key.text:
                    ...
                    Alter_der_Firma=checlslash(Alter_der_Firma)
                elif "Umsatz in CHF" in key.text:
                    Umsatz_in_CHF=key.find_next_sibling('span').text
                    Umsatz_in_CHF=checlslash(Umsatz_in_CHF)
                elif "Mitarbeiter" in key.text:
                    Mitarbeiter=key.find_next_sibling('span').text.strip()
                    Mitarbeiter=checlslash(Mitarbeiter)
                    if Mitarbeiter!="":
            ...
            status_div=soup.find("div",class_="company-status company-info-block")
            status="Active"
            if status_div:
                if 'gelöscht' in status_div.text:
                    status="Deleted"
                else:
                    status="Active"
            #Company Name
            Company_name=""
            Cname=soup.find('div',class_="company-name")
            if Cname:
                Company_name=Cname.text.strip()
            # print(Company_name)
            #Get All Address Manually
            main_span_address=soup.find('span',class_="overview-item-text")
            plz=ort=Adresse=""
            if main_span_address:
                a_rem=main_span_address.find("a",recursive=False)
                if a_rem:
                    a_rem.decompose()
                parts = [part.strip('" ') for part in main_span_address.stripped_strings if not part.startswith('Nachbarschaft')]
                if len(parts)==3:
                    Adresse=f"{parts[0]},{parts[1]}"
                    partslist=separate_number_and_text(parts[2])
                    plz=partslist[0]
                    ort=partslist[1]
                if len(parts)==2:
                    Adresse=parts[0]
                    partslist=separate_number_and_text(parts[1])
                    plz=partslist[0]
                    ort=partslist[1]
                    # detail_address=parts[1].split(" ")
                    # print(len(detail_address))
                    # if len(detail_address)==1:
                    #     ort=parts[0].strip()
                    # else:
                    #     plz=detail_address[0].strip()
                    #     ort=parts[1].replace(plz,"").strip()
                elif len(parts)==1:
                    # detail_address=parts[0].split(" ")
                    partslist=separate_number_and_text(parts[0])
                    plz=partslist[0]
                    ort=partslist[1]
                    # print(len(detail_address))
                    # if len(detail_address)==1:
            ...
            all_h2=soup.find_all('h2',class_="t-subtitle")
            type="Hauptsitz"
            for h2 in all_h2:
                if h2.text.strip()=="Hauptsitz":
                    type="Zweigniederlassungen"
            ...
            obj2={
                ...
                "managment_type":managment_type,
                "Branch/HeadQuarter": type,
            }
            return obj2
        else:
...
ScrapedData=readCSV("unmatched.csv")
Main_list=[]
MissingData=[]
with ThreadPoolExecutor(max_workers=50) as executor:
    ...
    for index, obj in enumerate(ScrapedData):
        # if index>=0 and index<=50:
            futures.append(executor.submit(retriveIndividualPageswithAdditionalData, obj,index))
            # break
...
df = df.astype(str)
df.to_csv(f"TestingFinalData{program}.csv", index=False)
...
df1 = df1.astype(str)
df1.to_csv(f'testingMissingFinal{program}.csv', index=False)
# print(list1[0]["Url"])