...
#                 json.dump(outright, f, ensure_ascii=False, indent=4)
...
"""
Scraper for Netbet, Genybet, and Olybet - Final Formatting
"""
...
def clean_bet_from_label(label):
    """
    Cleans the label to keep Match + Bet, removing the date prefix and trailing '?'
    Input:  "Super Cote 18/01/2026 - Sénégal/Maroc - Le Maroc marque au moins 2 buts ?"
    Output: "Sénégal/Maroc - Le Maroc marque au moins 2 buts"
    """
    if not label:
        return "Unknown"

    # 1. Remove the "Super Cote DD/MM/YYYY - " prefix
    # We look for the first occurrence of " - " and discard everything before it if it contains "Super Cote"
    # This keeps "Sénégal/Maroc - Le Maroc marque au moins 2 buts ?"
    clean_label = re.sub(r'^Super Cote \d{2}/\d{2}/\d{4}\s*-\s*', '', label)

    # 2. Remove the trailing question mark '?'
...
def parse_outright_to_rows(outright, site_name):
    ...
    for event in outright.get("events", []):
        full_label = event.get("label", "Unknown")
        
        # --- TIME ---
        start_time_raw = event.get("start", "")
        start_time = parse_iso_time(start_time_raw)
        
        # --- SPORT ---
        category_label = event.get("category", {}).get("label", "Unknown")
        sport = category_label.replace("Super Cote", "").strip()
        ...
        # --- BET ---
        # Updated cleaning logic applied here
        bet_val = clean_bet_from_label(full_label)
