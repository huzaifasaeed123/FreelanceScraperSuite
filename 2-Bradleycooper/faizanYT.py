...
def check_config():
    ...
    while True:
        try:
            response = requests.get("https://www.myexternalip.com/raw", proxies=proxy, timeout=(config["proxy_timeout"]))
            time.sleep(0.5)
        except:
    ...
    for i in range(len(config["accounts-1k-10k"])):
        ...
        while True:
            try:
                response = requests.post(endpoint, headers=headers, json=json, timeout=10)
            except:
                proxy_fails = proxy_fails+1
                if proxy_fails > config["max_proxy_fails"]:
                    print(f'{Fore.RED}[ERROR] Proxy not working, too many proxy fails')
...
def send_claim_request(username, password, views, thread_number):
    ...
    while True:
        ...
        try:
            response = requests.post(endpoint, headers=headers, json=json, proxies=proxy, timeout=(config["proxy_timeout"]))
        except:
            proxy_fails = proxy_fails+1
            if proxy_fails > config["max_proxy_fails"]:
