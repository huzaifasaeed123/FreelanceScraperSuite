...
# All existing parser functions from db_parser.py
...
def parse_all_data(raw_table, parsed_table, is_parts=False, batch_size=1000, output_excel=None):
    """Main function to parse all data from raw table to parsed table"""
    all_results = []
    ...
    for batch in get_records_in_batches(raw_table, batch_size):
        ...
        # Create args with is_parts flag
        ...
        if results:
            parsed_table.insert_many(results)
