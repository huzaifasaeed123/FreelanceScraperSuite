...
db = dataset.connect('sqlite:///capterra_top_ten.db')
review_table = db['listings']  #review
...
for single_company in unscraped_rows:
    ...
    vendor=single_company["Vendor"]
    # main_comapny_div=soup.find("div",{"data-testid":"product-header"})
    # print(main_comapny_div)
    # if main_comapny_div:
    #     vendor=main_comapny_div.find("h1").text.replace("Reviews of","")
    #     rating_span=main_comapny_div.find("span",class_="star-rating-label").text.strip()
    #     overall_rating=rating_span.split("(")[0]
        # print(vendor,overall_rating)
        # reason_for_choosing=reason_for_choosing.text.strip()
