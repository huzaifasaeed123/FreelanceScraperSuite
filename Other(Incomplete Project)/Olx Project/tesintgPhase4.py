...
code_challenege = parse_qs(parsed_url.query).get('code_challenge')[0]
code_challenege_method = parse_qs(parsed_url.query).get('code_challenge_method')[0]
redirected_url = parse_qs(parsed_url.query).get('redirect_uri')[0]
state = parse_qs(parsed_url.query).get('state')[0]
st = parse_qs(parsed_url.query).get('st')[0]
print("cc Code:", cc)
print("client_id",client_id)
print(code_challenege,code_challenege_method)
print("Redirect URL",redirected_url)

authenticate_url=f"https://login.olx.ro/api/initiate-auth?client_id={client_id}&redirect_uri={redirected_url}&code_challenge={code_challenege}&code_challenge_method={code_challenege_method}&state={state}%3D%3D&st={st}"
...
response=session.post(authenticate_url,json=payload,headers=headers)
